...
(function ( $ ) {
  ...
  $.fn.mulang = function( options ) {
    ...
    if(!__mulang[options.lang] && navigator.languages){
      ...
      loop: for (var i = 1; i < languageList.length; i++) {
        ...
        for (var j = 0; j < Object.keys(__mulang).length; j++) {
          if(Object.keys(__mulang)[j] == lang) {
    ...
    this.each(function(){
      ...
      var lang_key_array = mulang_tag.split(';');

      for (var i = 0; i < lang_key_array.length; i++) {
        var lang_key = lang_key_array[i];
        var attr = 'text';
        //find array
        var param_arr = null;
